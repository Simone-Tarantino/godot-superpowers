...
```
.
├── .claude-plugin/plugin.json    # plugin manifest. `skills` MUST be declared as an **array of individual skill paths** (e.g. `["./skills/foo", "./skills/bar"]`) — empirically NOT auto-discovered (v1.12.2 attempted to omit the field; result was 0 skills registered, fixed in v1.12.3). `agents/`, `hooks/hooks.json`, and `.mcp.json` ARE auto-discovered by convention and must NOT be declared in the manifest. Earlier versions (≤1.12.1) shipped `"skills": "./skills/"` as a string — the loader expects an array, so string values are silently dropped (zero skills registered). Validator now enforces array-of-paths matching the on-disk skill directories.
...
├── skills/                       # 33 skills (each <name>/SKILL.md) — every directory MUST be listed in plugin.json `skills` array (validator enforces 1:1 parity)
...
├── .mcp.json                     # recommended MCP servers — auto-discovered by convention (no explicit field in plugin.json). Strict to the MCP schema: no `_tier` / `_purpose` annotations inline.
