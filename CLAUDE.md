...
```
.
├── .claude-plugin/plugin.json    # plugin manifest — metadata only (name, version, author, keywords). `skills/`, `agents/`, `hooks/hooks.json`, and `.mcp.json` are ALL auto-discovered by Claude Code's plugin-mode convention; do NOT add explicit `skills` / `mcpServers` fields. Earlier versions shipped `"skills": "./skills/"` and `"mcpServers": "./.mcp.json"` as strings, but the loader's schema treats `skills` as an array of individual skill paths — string values were silently dropped, so the plugin loaded with zero registered skills. Fixed in v1.12.2.
...
├── skills/                       # 33 skills (each <name>/SKILL.md) — auto-discovered (no explicit field in plugin.json)
